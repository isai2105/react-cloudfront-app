...
function noForbiddenElements(html) {
  ...
  const problems = []
  if (
    tags(html, 'link').some((attrs) => attributeValue(attrs, 'rel')?.toLowerCase() === 'manifest')
  ) {
    problems.push('<link rel="manifest">')
  }
  if (
    tags(html, 'meta').some(
      (attrs) => attributeValue(attrs, 'http-equiv')?.toLowerCase() === 'content-security-policy',
    )
  ) {
    ...
    for (const attrs of tags(html, name)) {
      for (const attr of ['src', 'href', 'srcset']) {
        ...
        for (const url of attr === 'srcset' ? srcsetUrls(value) : [value]) {
          if (!isAllowedOrigin(name, url))
            problems.push(`<${name} ${attr}="${url}"> is not same-origin`)
        }
      }
    }
  }
...
async function noDataUrisOutsideImages(files) {
  ...
  return texts.flatMap((text, i) => {
    ...
    const hits = new Set(
      [
        ...text.matchAll(
          /data:(font\/[\w+.-]+|text\/css|application\/javascript|text\/javascript)/gi,
        ),
      ].map((m) => m[1]),
    )
    return [...hits].map((type) => `${relative(DIST, file)} contains a data:${type} URI`)
  })
}
...
async function buildInfoIsStamped() {
  ...
  let parsed
  try {
    parsed = JSON.parse(await readFile(join(DIST, 'build-info.json'), 'utf8'))
  } catch (error) {
    return [`build-info.json unreadable: ${error instanceof Error ? error.message : String(error)}`]
  }
  if (typeof parsed !== 'object' || parsed === null) return ['build-info.json is not a JSON object']
  const { sha, builtAt } = /** @type {Record<string, unknown>} */ (parsed)
  /** @type {string[]} */
  const problems = []
  if (typeof sha !== 'string' || !COMMIT_SHA.test(sha)) {
    problems.push(`build-info.json sha is not a 40-hex commit id: ${JSON.stringify(sha)}`)
  }
  if (typeof builtAt !== 'string' || Number.isNaN(Date.parse(builtAt))) {
    problems.push(`build-info.json builtAt is not an ISO-8601 date: ${JSON.stringify(builtAt)}`)
  }
  return problems
}
...
const textFiles = (await readdir(DIST, { recursive: true, withFileTypes: true }))
  .filter((entry) => entry.isFile() && CSP_TEXT_TYPES.includes(extname(entry.name).slice(1)))
